...
The package also exports `bootstrapFewShot` and `labeledFewShot` for installing examples from teacher traces or labeled data. Unlike their DSPy counterparts they score the compiled workflow over the training set (one evaluation pass), so all four optimizers return the same `{ candidates: [prompts, { score }][], score }` shape. `bootstrapFewShot` also takes an optional `gate: { scorer, threshold? }` — a Mastra `type: "trajectory"` scorer that receives each teacher rollout as a Trajectory in `run.output` and decides which rollouts become demos, while the objective `scorer` still scores the compiled workflow. The Trajectory comes from Mastra's own `extractWorkflowTrajectory`, so it covers every step the engine executed — agents, tools, and plain steps included, a nested workflow as a single entry — with loop iterations collapsed to one entry per step id. Per-step inputs aren't recorded (Mastra's Trajectory step carries no input field): the workflow input sits at `rawWorkflowResult.stepResults.input`, and each later step's input is the prior entry's output.
